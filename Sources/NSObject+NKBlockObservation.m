#import "NSObject+NKBlockObservation.h"
...
@private
    __weak id observee;
    NSString *keyPath;
    NKObservationBlock task;
    NSOperationQueue *queue;
    dispatch_once_t cancellationPredicate;    
}

- (AMObserverTrampoline *)initWithObservingObject:(id)obj keyPath:(NSString *)keyPath onQueue:(NSOperationQueue *)queue task:(NKObservationBlock)task;
...
- (AMObserverTrampoline *)initWithObservingObject:(id)obj keyPath:(NSString *)newKeyPath onQueue:(NSOperationQueue *)newQueue task:(NKObservationBlock)newTask {
...
@implementation NSObject (NKBlockObservation)

- (NSString *)addObserverForKeyPath:(NSString *)keyPath task:(NKObservationBlock)task {
    return [self addObserverForKeyPath:keyPath onQueue:nil task:task];
}

- (NSString *)addObserverForKeyPath:(NSString *)keyPath onQueue:(NSOperationQueue *)queue task:(NKObservationBlock)task {
